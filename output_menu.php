...
if(!empty($_POST)){
    ...
    }elseif($_POST["type"]==="freee"){
        $sqlstr="select '収入' as 収支,CURDATE()+0,Uridate,'売上高' as 科目,sum(UriageKin) as 売上,'外税',sum(zei) as 税額,CONCAT('売上No:',UriageNo) as 備考,'事業主貸' as 決済口座";
        $sqlstr=$sqlstr." from UriageData where uid=? and Uridate between ? and ?";
...
<body class='common_body'>
    ...
    <form method='post' action='#' style="font-size:1.5rem">
        ...
        <select class='form-select' style="font-size:1.5rem;padding:0;max-width:400px;" name='type' id='type'>
            <option value='yayoi'>やよいの青色申告 オンライン</option>
            <option value='freee'>freee会計(確定申告)</option>
        </select>
        <br>
        <input class='btn btn-primary' type='submit' value='CSV出力' style='width:200px;height:100px;'>
    </form>
    </div>
