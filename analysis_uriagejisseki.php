...
if($_POST["sum_tani"]==1){//日ごと
    ...
    $aryColumn = ["商品名","売上数"];
}elseif($_POST["sum_tani"]==6){//イベント・店舗別客単価ランキング
    /*客単価一覧
    $sqlstr = "select 計上日,Event,ROUND(avg(税抜売上)) as 客単価 from ";
...
<body style='padding-top:55px'>
    ...
    <div class="row">
    <div class="col-md-3" style='padding:5px;background:gray'>
        <form class="form" method="post" action="analysis_uriagejisseki.php">
            <select name='ymfrom'>
            <?php
            foreach($SLVresult as $row){
                //if((string)date('Y')."01"==$row["Value"]){
                if($ymfrom==$row["Value"]){
            ...
            <select name='sum_tani'><!--集計単位-->
                <option value='1' <?php if($_POST["sum_tani"]==1){echo "selected";} ?> >日ごと</option>
                <option value='2' <?php if($_POST["sum_tani"]==2 || empty($_POST["sum_tani"])){echo "selected";} ?>>月ごと</option>
                <option value='3' <?php if($_POST["sum_tani"]==3){echo "selected";} ?> >年ごと</option>
                ...
                <option value='6' <?php if($_POST["sum_tani"]==6){echo "selected";} ?> >客単価ランキング</option>
            </select>
            <select name='list'>
            ...
            ?>
            <input type='submit' class='btn-view' style='padding:0;hight:10px;width:80px' value='検 索'>
        </form>
    </div>
